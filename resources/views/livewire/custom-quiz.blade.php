    ...
    @else
        <div class="flex flex-col items-center mt-6 mb-6 w-full">
            @if($freshResult)
                <div class="mt-2 mb-6 flex flex-col items-center">
                    @if($lastResults==$maxRounds)
                        <h3 class="text-green-700 text-4xl mb-4">{{ __('Congratulations!') }}</h3>
                    @endif
                    <h2 class="my-2 font-semibold text-2xl text-gray-600 leading-tight"> {{ __('Result:') }}</h2>
            ...
            <x-jet-button class="bg-houseColor mx-auto">
                <a href="#" wire:click.prevent="startCustomQuiz">{{ __('Start a round') }}</a>
            </x-jet-button>
        </div>
    @endif
</div>
